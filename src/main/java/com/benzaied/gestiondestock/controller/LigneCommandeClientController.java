...
@RestController
@RequestMapping(path = "/gestiondestock/v1/ligneCommandeClients")
@AllArgsConstructor
@Validated
public class LigneCommandeClientController {
    ...
    @Operation(
            summary = "Create a new line item for a client order",
            responses = {
                    @ApiResponse(responseCode = "201", description = "Line item created successfully", content = @Content(schema = @Schema(implementation = LigneCommandeClientDto.class)))
            }
    )
    public ResponseEntity<LigneCommandeClientDto> save(@Valid @RequestBody LigneCommandeClientDto ligneCommandeClientDto) {
        LigneCommandeClientDto savedLigneCommandeClient = ligneCommandeClientService.save(ligneCommandeClientDto);
        emailSenderService.sendSimpleEmail();
        return ResponseEntity
                .status(HttpStatus.CREATED)
                .body(savedLigneCommandeClient);
    }
    ...
    public ResponseEntity<LigneCommandeClientDto> findById(@PathVariable Integer id) {
        LigneCommandeClientDto ligneCommandeClientDto = ligneCommandeClientService.findById(id);
        return ResponseEntity.status(HttpStatus.OK).body(ligneCommandeClientDto);
    }
    ...
    public ResponseEntity<List<LigneCommandeClientDto>> findAll() {
        List<LigneCommandeClientDto> ligneCommandeClientDtos = ligneCommandeClientService.findAll();
    ...
    public ResponseEntity<ResponseDto> delete(@PathVariable Integer id) {
        boolean isDeleted = ligneCommandeClientService.delete(id);
        if (isDeleted) {
            return ResponseEntity
